...
if(isset($_POST['update'])){
    ...
    $updt="UPDATE `tblpublicnotice` SET `Notice_Title`='$notice',`Notice_Message`='$nmessage', WHERE sno=$id";
...
if(isset($_GET['sno'])){
    $id1=$_GET['sno'];
    $read="select * from tblpublicnotice where sno='$id1'";
...
<body>
     ...
     <div class="container-fluid" style="background-color:#ecf0f4">
        <div class="row">
            ...
            <div class="row">
              <div class="col-sm-9">
                <h4 class="ps-3">Update Public Notice</h4>
              </div>
              <div class="col-sm-3">
...
<form method="post" class="bg-white">
  <h4 class="text-center">Update Public Notice</h4>
  <div class="mb-3 ps-3 pe-3">
    <label for="exampleInputnotice" class="form-label">Notice Title</label>
    <input type="text" class="form-control" id="exampleInputnotice" aria-describedby="noticeHelp" name="notice" value="<?php echo $row['Notice_Title']?>"> 
  </div>
  <div class="mb-3 ps-3 pe-3">
...
<script>
function myFunction() {
  window.alert('Public Notice has been updated');
}
</script>
</body>
</html>